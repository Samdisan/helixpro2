<?php
// act1_full_access.php — акт 1, Олімп: «Повний доступ» → зниження рівня до 1 (доступ падає до 1 рівня)
...
foreach ($users as $i => $u) {
    if (($u['access_code'] ?? '') === $code) {
        ...
        if (!empty($u['act1_full_access_used'])) {
            echo json_encode(['ok' => true, 'already_used' => true, 'message' => 'Вже виконано']);
            exit;
        }
        $users[$i]['level'] = '1';
        $users[$i]['act1_full_access_used'] = true;
        $found = true;
        break;
    }
}
